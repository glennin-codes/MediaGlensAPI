...
const StorageInfoCard = () => {
  ...
  const handleShowSnackbar = (message, type) => {
...
console.log(bytes);
  ...
  const usedSpaceDisplay = usedSpaceGB >= 1 ? `${usedSpaceGB.toFixed(3)} GB` : `${(usedSpaceGB * 1024).toFixed(3)} MB`;
  const remainingSpaceDisplay = remainingSpaceGB >= 1 ? `${remainingSpaceGB.toFixed(3)} GB` : `${(remainingSpaceGB * 1024).toFixed(3)} MB`;
  ...
  const progressBarColor = remainingSpaceGB <= warningThresholdMB ? 'red' : 'green';
...
<div className=" w-full h-full ">
  ...
  <div className="max-w-md mx-auto flex flex-col justify-center  rounded-xl overflow-hidden md:max-w-2xl mt-10 shadow-md p-5">
      ...
      <p className="small-text text-lg px-2">Total Capacity: {totalCapacityGB} GB</p>
      <p className="small-text text-lg">Total Items Uploaded: {storageInfo.totalItems}</p>
      <p className="small-text">Used Space: {usedSpaceDisplay}</p>
      <p > <span className="small-text">Remaining Space:  </span>
      <span className={`${remainingSpaceGB <= warningThresholdMB ? 'text-red-400' : 'text-blue-400/100 '}`}>
